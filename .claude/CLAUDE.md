...
# Third-Party Application Registry data (real Georgia Southern vendor security/
# privacy review records, fetched live from a Google Sheet the GS IT team
# maintains by hand). Never includes staff/vendor contact emails -- stripped
# at parse time, not just at display time. Powers thirdpartyapps.html.
python3 server/scripts/build_thirdparty_apps_data.py
# → server/data/thirdparty_apps/{apps,meta}.json (gitignored)
# → client/data/thirdpartyapps/{apps,meta}.json (same files, copied for the
...
- **`client/thirdpartyapps.html` (Third-Party Application Registry) is a fifth static page, unrelated to the GOSA K-12 dataset the other four pages share** — it tracks Georgia Southern University's own real vendor security/privacy review records (product name, review status history, GDPR/FERPA/COPPA compliance claims, HECVAT/SOC 2 attestations, cited policy URLs, and ~29 rubric-question answers spanning Data Collection/Security/Third Party Data/Advertising/USG IT Handbook), sourced from a Google Sheet the GS IT team maintains by hand and pulled in by `build_thirdparty_apps_data.py`. Unlike the other four pages it is single-brand (Georgia Southern only, `data-brand="georgia-southern"` hardcoded, no dev-preview theme toggle) since it isn't a multi-tenant product. **The sheet's own `RUBRIC: *`/`CERTIFIED` rollup columns are all literally `"NOT YET SCORED"` for every row as of this writing** — the page does not invent a rollup; it computes each app's rubric-category pie charts client-side from the real per-question answers (`Meets`/`Partially`/`Unmet`/`N/A`/`Unknown`/`Not yet scored`) instead, so a heavy grey "Not Scored" slice on most apps reflects real review backlog, not a bug. The `rating` badge (Approved/Renewal Pending/Expired/Denied/Under Review/Unrated) is **derived**, not sourced directly — `Status`/`Assessment History` in the sheet are the same pipe-separated append log (verified: identical value distributions), and the pipeline script treats the rightmost segment as current and maps it via `STATUS_TO_RATING`; the full segment history is kept and shown too, so the derivation is auditable rather than opaque. **Staff and vendor-contact email columns (`Original Requester Email`, `Vendor Contact Email`) are dropped at parse time, in both the gitignored server copy and the committed client copy** — real Georgia Southern staff emails must never reach `client/data/`, which is published to the public GitHub Pages site. Institution-added data (Additional Info notes/licensing/tags, custom private apps, hidden-app list, manual rating overrides, a Users roster, and a Changelog audit trail) is **not** in the pipeline output at all — it lives entirely in `localStorage` (key `thirdPartyAppRegistry.overlay.v1`), same pattern as `slides.html`'s deck builder (JSON export/import for backup, a quota-exceeded warning banner), since none of that is real sourced data and it's per-institution/per-browser by nature. The Users list starts empty — no fabricated staff roster.
...
- **Live demo**: `client/` (including `client/data/milestones/`, `client/data/regression/`, `client/data/enrollment/`, `client/data/thirdpartyapps/`, and `client/vendor/pptxgen.bundle.js`) is pushed to GitHub and served via GitHub Pages at `https://nic-carroll.github.io/Georgia-School-Data/client/` (drilldown), `.../client/regression.html` (regression), `.../client/schoolprofile.html` (school profile), `.../client/slides.html` (slide deck builder), and `.../client/thirdpartyapps.html` (third-party application registry) — the URL includes `/client/` because Pages serves from the repo root and there's no build step that flattens it. `client/data/milestones/`, `client/data/regression/`, `client/data/enrollment/`, `client/data/thirdpartyapps/`, and `client/vendor/pptxgen.bundle.js` are the exceptions to the "raw/derived data stays local" pattern elsewhere in this file — they're committed specifically so Pages has something to serve. `client/data/thirdpartyapps/apps.json` never contains staff/vendor emails (stripped at parse time — see Architecture Quirks) precisely because this directory is public. After any push, force a rebuild via `POST /repos/nic-carroll/Georgia-School-Data/pages/builds` (GitHub's legacy Pages builder does not reliably auto-trigger on push) and poll `GET .../pages/builds/latest` until `status: "built"` before assuming the live site reflects the new commit.
...
- `server/data/gosa/*.json`, `server/data/gosa/milestones_dashboard/*.json`, `server/data/gosa/regression/*.json`, `server/data/gosa/enrollment_profile/*.json`, and `server/data/thirdparty_apps/*.json` — Auto-generated outputs. Never hand-edit.
...
Georgia Southern vendor-review Google Sheet (separate from GOSA above)
    │ build_thirdparty_apps_data.py (fetch CSV export over HTTP, strip emails)
    ▼
client/data/thirdpartyapps/{apps,meta}.json ── fetched on demand by
client/thirdpartyapps.html (Additional Info/custom apps/users/changelog are
NOT in this file -- they live in that page's own localStorage overlay)
```
